...
(function(){
  ...
  const makeContextChecks=({label,title,text,equation,result})=>{
    ...
    return [
    [`${context}。解决这个具体问题，哪种做法最合适？`,method,'只看数字大小，不读题意','把所有数字直接相加',`${method}，再用${result}检查答案是否符合题意。`],
  ...
  window.mathUnit=function(u){
    ...
    const checks=Array.isArray(u.checks)&&u.checks.length?u.checks:makeContextChecks({label,title,text,equation,result});
    const method=`先根据题目条件，确定数量或图形关系，再完成：${equation}`;
    return {
      ...u,
      life,checks,
      story:{label,title,text,equation,rule:method},
      activity:{title:'选对方法再出发',text:`遇到“${title}”这样的任务，哪种做法更合适？`,model:u.icon,options:[method,'只看最后一个数字就猜','把所有数字直接相加'],answer:0,success:`选对了：${method}`},
